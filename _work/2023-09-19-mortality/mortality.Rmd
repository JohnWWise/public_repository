...
```{r import, echo=FALSE, code_folding = TRUE}
...
cords <- read_csv("/Users/johnwise/pubic_repository/csvs/fdoc_mortality/facility_cords.csv")
fy15 <- read_csv('/Users/johnwise/pubic_repository/csvs/fdoc_mortality/FY 2015-2016.csv', col_names = TRUE, skip=1, show_col_types = FALSE)
fy15$`DC Number` <- str_extract(fy15$`DC Number`, "(?<=DCNumber=)[^&]+")
fy16 <- read_csv('/Users/johnwise/pubic_repository/csvs/fdoc_mortality/FY 2016-2017.csv', col_names = TRUE, skip=1, show_col_types = FALSE)
fy16$`DC Number` <- str_extract(fy16$`DC Number`, "(?<=DCNumber=)[^&]+")
fy17 <- read_csv('/Users/johnwise/pubic_repository/csvs/fdoc_mortality/FY 2017-2018.csv', col_names = TRUE, skip=1, show_col_types = FALSE)
fy17$`DC Number` <- str_extract(fy17$`DC Number`, "(?<=DCNumber=)[^&]+")
fy18 <- read_csv('/Users/johnwise/pubic_repository/csvs/fdoc_mortality/FY 2018-2019.csv', col_names = TRUE, skip=1, show_col_types = FALSE)
fy18$`DC Number` <- str_extract(fy18$`DC Number`, "(?<=DCNumber=)[^&]+")
fy19 <- read_csv('/Users/johnwise/pubic_repository/csvs/fdoc_mortality/FY 2019-2020.csv', col_names = TRUE, skip=1, show_col_types = FALSE)
fy19$`DC Number` <- str_extract(fy19$`DC Number`, "(?<=DCNumber=)[^&]+")
fy20 <- read_csv('/Users/johnwise/pubic_repository/csvs/fdoc_mortality/FY 2020-2021.csv', col_names = TRUE, skip=0, show_col_types = FALSE)
colnames(fy20) <- names(fy19)
fy20$`DC Number` <- str_extract(fy20$`DC Number`, "(?<=DCNumber=)[^&]+")
fy21 <- read_csv('/Users/johnwise/pubic_repository/csvs/fdoc_mortality/FY 2021-2022.csv', col_names = TRUE, show_col_types = FALSE, skip=1)
fy21$`DC Number` <- str_extract(fy21$`DC Number`, "(?<=DCNumber=)[^&]+")
fy22 <- read_csv('/Users/johnwise/pubic_repository/csvs/fdoc_mortality/FY 2022-2023.csv', col_names = TRUE, skip=1, show_col_types = FALSE)
fy22$`DC Number` <- str_extract(fy22$`DC Number`, "(?<=DCNumber=)[^&]+")
fy23 <- read_csv('/Users/johnwise/pubic_repository/csvs/fdoc_mortality/FY 2023-2024.csv', col_names = TRUE, skip=1, show_col_types = FALSE)
fy23$`DC Number` <- str_extract(fy23$`DC Number`, "(?<=DCNumber=)[^&]+")
```

```{r preprocessing, echo=FALSE, code_folding = TRUE}
...
deaths$FACILITY_description <- as.factor(deaths$FACILITY_description)
...
```{r cleaning: collapsing factors, echo=FALSE, code_folding = TRUE}
# Manner of Death
...
```{r plotting all, echo=FALSE, fig.align='center', fig.height=5, fig.width= 10, warning=FALSE, code_folding = TRUE}
per_month_all <- deaths %>%
  group_by(month = floor_date(Date_Death, unit = "month")) %>%
  summarise(Total_month_Deaths = n())

plot_all <- ggplot(per_month_all, aes(x = as.Date(month), y = Total_month_Deaths)) + 
  geom_rect(aes(xmin = as.Date("2020-03-09"), xmax = as.Date("2021-08-25"), ymin = -Inf, ymax = Inf), fill = "lightpink", alpha = 0.3) +
  geom_col(fill = "blue") + 
  guides(color = 'none') +
  ggtitle("FDOC Dates of Death July 2015 - June 2023") +
  scale_x_date(date_labels = "%B, %Y", date_breaks = "1 year") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  coord_cartesian(xlim = as.Date(c("2015-07-02", "2023-06-29"))) +
  geom_text(aes(label = Total_month_Deaths), vjust = 1.6, color = "white", size = 1.5) +
  xlab(NULL) +
  ylab('Count')
```
...
```{r plotting RMC, echo=FALSE, fig.align='center', fig.height=5, fig.width= 10, warning=FALSE, message=FALSE, code_folding = TRUE}
...
ggplot(per_month_rmc_keyed, aes(x = as.Date(month), y = Total_month_Deaths, fill = RMC)) + 
  ...
  xlab(NULL) +
  ylab('Count') +
  labs(fill = "", title = "FDOC Deaths -- RMC/Not-RMC:") +
...
```{r plotting homicide facilities, echo=FALSE, fig.align='center', fig.height=5, fig.width= 10, warning=FALSE, message=FALSE, code_folding = TRUE}
violent_FDOC <- deaths %>%
...
# Accidental Deaths
```{r id_accident_weeks, echo=FALSE, code_folding = TRUE}
...
```{r plotting p_valued weeks2, echo=FALSE, fig.align='center', fig.height=5, fig.width= 10, warning=FALSE, message=FALSE, code_folding = TRUE}
...
```{r echo=TRUE, fig.align='center', fig.height=5, fig.width= 10, warning=FALSE, message=FALSE, code_folding = TRUE}
...
The following weeks, filtered to exclude weeks during the 2020-2021 COVID emergency in Florida, stand out as exceptional in the FDOC with regards to the amount of natural deaths.
...
#kable(summary_data1 %>% filter(!(month >= as.Date("2020-03-09") & month <= as.Date("2021-08-25")))) %>%
...
In the following analysis, we examine maps of groups of facilities that experienced months of statistically significant natural death counts. Any facilities which individually and independently experienced high death counts of statistical significance in the same month were plotted, as well as results that were not necessarily statistically significant but high enough to be considered when searching for spatial patterns linking significant results (p ≤ 0.25).

The plotting algorithm automatically fits the mapping as tightly as possible but keeps the size of the marker proportional to the magnitude of the mapped effect, in this case p-values (the calculated probability that a given week is 'usual'). It is important to note that, when viewing these maps, the sizes of the circles remain constant regardless of zoom level. Comparing the magnitude of effect across different maps is valid, but it should be done with caution, as the size of the marker does not correspond to an area of land covered.

It is not valid to compare colorations across maps at this time, as the colors of each map cover the spectrum of their respective associated data entries (this is a coding issue -- frustrating -- i hope to soon fix).
...
leaflet(data) %>%
  ...
  addCircleMarkers(
    ...
    labelOptions = labelOptions(
      noHide = TRUE,          # Keep the label open by default
      direction = "auto",      # Position of the label relative to the marker
      textOnly = FALSE,        # Display only text (no HTML content)
      textsize = "12px",
      opacity = 1,
      offset = c(0,0),
      style = list("white-space" = "nowrap", "font-weight" = "bold"))  # CSS styles for the label
    ) %>%
  addControl(
    position = "bottomleft",
...
kable(data %>% select(FACILITY_description,Total_month_naturalDeaths,p_values,high)) %>% 
  kable_styling(latex_options = "hold_position")
```

## 2016
### January 2016
```{r echo=FALSE, fig.align='center', fig.height=8, fig.width= 10, warning=FALSE, message=FALSE}
