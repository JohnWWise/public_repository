...
Regarding those currently incarcerated, we observe that they are serving time for a wide range of adjudication charges. There are 790 different charges for which these individuals are currently incarcerated.

```{r code_folding = TRUE}
datatable(unique(charges), options = list(scrollX = TRUE, pageLength = 10, lengthMenu = c(10, 20, 50)))
```
...
kable(rounded_percentages, 
      col.names = c("Percentage of Current Crimes for those with Repeat Commitments by Crime Category")) %>% 
  kable_styling(latex_options = "hold_position")
```
Categorizing crimes in this way is a time-consuming task that required me to make numerous decisions regarding their classification. These decisions often felt arbitrary, even to myself. It is important to understand that there are many other valid crime categorization systems that may be equally or even more effective. Additionally, please note that crime categories are constantly being reviewed and adjusted.

I encourage everyone to click the disclosure triangle icon next to "View Crime Category Mappings" to review the decisions I have made. If you identify any errors or inconsistencies, or if you would like to propose an alternative system, please contact me.
...
for (category in categories) {
  num_associated_rows <- result %>%
    ...
    nrow()
  proportion <- round(num_associated_rows / nrow(result), 3)
  result_df <- rbind(result_df, data.frame(Category = category, Percent = proportion * 100))
}
```
  </p>
  </details>
...
```{r}
kable(result_df, 
      col.names = c("Crime Category", "Percentage of FDOC Currently Incarcerated Persons with Repeat Commitments Associated with each Crime Category")) %>% 
  kable_styling(latex_options = "hold_position")
```

In the analysis, it is observed that among the individuals currently incarcerated in FDOC who have previously served time in FDOC, 32% have at least one Burglary, Theft, or Property crime associated with their current incarceration, while 44% have been associated with at least one crime from the violent crime category, and so on. The proportions sum to greater than 100 because many individuals are associated with multiple crime categories.

While I initially intended to analyze probation/parole violations among individuals currently incarcerated with a previous history of incarceration using this data, it is not currently possible to conduct a straightforward analysis. This is because the Florida Department of Corrections categorizes individuals who are currently incarcerated and have violated their probation/parole under the charge for which they were originally sentenced to probation.
