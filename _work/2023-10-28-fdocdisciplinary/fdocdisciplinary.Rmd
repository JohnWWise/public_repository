...
```{r fig.align= 'center', fig.height=16, fig.width=20, code_folding = TRUE, out.width='100%', layout="l-body-outset"}
...
disc %>%
  ...
  coord_flip() +
  theme(
    axis.text.y = element_text(hjust = 1, vjust = 0.5, face = "bold", size = 10.5),  # Make y-axis text bold
...
conf <- disc %>% 
  ...
  merge(root %>% select(INCARCERATION_SUPERVISION_ID, RACE_CODE), by = "INCARCERATION_SUPERVISION_ID") %>% select(-"INCARCERATION_SUPERVISION_ID")

ggplot(conf, aes(x = reorder(INCARCERATION_DISCIPLINARY_VIOLATION_TYPE_DESC, INCARCERATION_DISCIPLINARY_VIOLATION_TYPE_DESC, function(x) length(x)), fill = RACE_CODE)) +
  geom_bar() +
  xlab("Count of Active FDOC Disciplinary Action") +
  ylab("Count") +
  ...
  coord_flip() +
  theme(
    axis.text.y = element_text(hjust = 1, vjust = 0.5, face = "bold", size = 8.5),
    legend.title = element_blank(),
    legend.position = "bottom",
    plot.background = element_rect(fill = "#ECE5D8"),
    panel.grid.major = element_line(color = "gray", linewidth = 0.2),
